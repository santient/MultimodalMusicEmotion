...
if __name__ == "__main__":
    data_dir = sys.argv[1] # DEAM dataset dir
    arousal = pd.read_csv(os.path.join(data_dir, "annotations/annotations averaged per song/dynamic (per second annotations)/arousal.csv"))
    valence = pd.read_csv(os.path.join(data_dir, "annotations/annotations averaged per song/dynamic (per second annotations)/valence.csv"))
    meta_out = []
    emotion_out = []
    opensmile_out = []
    # bert_out = []
    for (i, a_row), (_, v_row) in tqdm.tqdm(zip(arousal.iterrows(), valence.iterrows()), total=arousal.shape[0]):
        ...
        # print(sid)
        opensmile = pd.read_csv(os.path.join(data_dir, f"features/{sid}.csv"), sep=";")[KEEP]
    ...
    torch.save(meta_out, os.path.join(data_dir, "tensors/meta.pt"))
    torch.save(emotion_out, os.path.join(data_dir, "tensors/emotion.pt"))
    torch.save(opensmile_out, os.path.join(data_dir, "tensors/opensmile.pt"))
